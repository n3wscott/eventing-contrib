...
import (
	...
	knchannel "knative.dev/eventing/pkg/channel"
...
type SubscriptionsSupervisor struct {
	...
	receiver   *knchannel.MessageReceiver
	dispatcher *knchannel.MessageDispatcher

	subscriptionsMux sync.Mutex
	subscriptions    map[knchannel.ChannelReference]map[subscriptionReference]*stan.Subscription
...
func NewDispatcher(natssURL, clusterID, clientID string, logger *zap.Logger) (*SubscriptionsSupervisor, error) {
	d := &SubscriptionsSupervisor{
		logger:        logger,
		dispatcher:    knchannel.NewMessageDispatcher(logger.Sugar()),
		connect:       make(chan struct{}, maxElements),
		natssURL:      natssURL,
		clusterID:     clusterID,
		clientID:      clientID,
		subscriptions: make(map[knchannel.ChannelReference]map[subscriptionReference]*stan.Subscription),
	}
	d.setHostToChannelMap(map[string]knchannel.ChannelReference{})
	receiver, err := knchannel.NewMessageReceiver(
		createReceiverFunction(d, logger.Sugar()),
		logger.Sugar(),
		knchannel.ResolveChannelFromHostHeader(knchannel.ResolveChannelFromHostFunc(d.getChannelReferenceFromHost)))
	if err != nil {
		return nil, err
	}
	d.receiver = receiver
...
func createReceiverFunction(s *SubscriptionsSupervisor, logger *zap.SugaredLogger) func(knchannel.ChannelReference, *knchannel.Message) error {
	return func(channel knchannel.ChannelReference, m *knchannel.Message) error {
...
func (s *SubscriptionsSupervisor) UpdateSubscriptions(channel *messagingv1alpha1.Channel, isFinalizer bool) (map[eventingduck.SubscriberSpec]error, error) {
	...
	cRef := knchannel.ChannelReference{Namespace: channel.Namespace, Name: channel.Name}
...
func (s *SubscriptionsSupervisor) subscribe(channel knchannel.ChannelReference, subscription subscriptionReference) (*stan.Subscription, error) {
	s.logger.Info("Subscribe to channel:", zap.Any("channel", channel), zap.Any("subscription", subscription))

	mcb := func(msg *stan.Msg) {
		message := knchannel.Message{}
		...
		if err := s.dispatcher.DispatchMessage(&message, subscription.SubscriberURI, subscription.ReplyURI, knchannel.DispatchDefaults{Namespace: channel.Namespace}); err != nil {
...
func (s *SubscriptionsSupervisor) unsubscribe(channel knchannel.ChannelReference, subscription subscriptionReference) error {
...
func getSubject(channel knchannel.ChannelReference) string {
	return channel.Name + "." + channel.Namespace
}

func (s *SubscriptionsSupervisor) getHostToChannelMap() map[string]knchannel.ChannelReference {
	return s.hostToChannelMap.Load().(map[string]knchannel.ChannelReference)
}

func (s *SubscriptionsSupervisor) setHostToChannelMap(hcMap map[string]knchannel.ChannelReference) {
...
func (s *SubscriptionsSupervisor) UpdateHostToChannelMap(ctx context.Context, chanList []messagingv1alpha1.Channel) error {
	hostToChanMap, err := knchannel.NewHostNameToChannelRefMap(chanList)
	if err != nil {
...
func (s *SubscriptionsSupervisor) getChannelReferenceFromHost(host string) (knchannel.ChannelReference, error) {
